...
function get_user_tasks(id) {
                ...
                <div class="col-12 border border-secondary rounded p-3 shadow">
                        ...
                        <div class="col-2">
                                <select id="status_${tarefa.id_tarefa}" onchange="change_task_status(${tarefa.id_tarefa})" class="form-select p-2 ${color.select_bg_color}">
                                    <option value="Nova" ${tarefa.status == 'Nova_task' ? 'selected' : ''}>Nova</option>
    ...
    fetch(`http://localhost:3000/user/${id}/tasks/total_tasks`)
        .then(response => {
            if (response.status === 200) {
                return (response.json());
            } else {
...
document.querySelector("#btn_nova_tarefa").addEventListener('click', () => {
    ...
    const url = window.location.origin + "/Projeto/ToDoList/frontend/novatarefa.html?id_usuario=" + id_usuario;
    window.location.href = url;
})