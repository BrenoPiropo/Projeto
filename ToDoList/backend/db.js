...
app.get("/user/:id/tasks/", (req, res) => {
    connection.query("SELECT * FROM tarefas WHERE id_usuario=? ORDER BY CASE WHEN status = 'Nova' THEN 1 WHEN status = 'Pendente' THEN 2 WHEN status = 'Cancelada' THEN 3 WHEN status = 'Concluída' THEN 4 ELSE 5 END;", [req.params.id], (err, results) => {
        if (err) {
            res.send('MYSQL erro');
        }
        res.json(results);
...
app.get("/user/:id/tasks/total_tasks/", (req, res) => {
